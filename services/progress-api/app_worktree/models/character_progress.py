...
class CharacterProgress(Base):
    ...
    # Roster fields (added in migration 002)
    user_bnet_id = Column(Integer, nullable=True, index=True)  # Links character to Battle.net account
    level = Column(Integer, nullable=True)
    avatar_url = Column(String(512), nullable=True)
    display_order = Column(Integer, server_default="0")  # Ordering within roster view

    # Gear progression
    current_ilvl = Column(Float, nullable=True)
    gear_details = Column(JSON, nullable=True)  # Full gear breakdown from Blizzard API
    last_gear_sync = Column(DateTime(timezone=True), nullable=True)  # Last Blizzard API sync
    parsed_gear = Column(JSON, nullable=True)  # Structured 16-slot gear format
    character_stats = Column(JSON, nullable=True)  # Primary + secondary stats
