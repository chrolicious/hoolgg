...
export default function RosterPage() {
  ...
  const handleDrop = useCallback(
    (targetIndex: number, zone: DropZone) => {
      ...
      setCharacters((prev) => {
        ...
        setTimeout(async () => {
          try {
            await progressApi.post(`/users/me/characters/reorder`, {
  ...
  const handleSyncAll = useCallback(async () => {
    ...
    try {
      await progressApi.post(`/users/me/characters/${characters[0].id}/gear/sync-all`); // Note: Backend needs to support this without guildId
  ...
  }, [isSyncingAll, characters, fetchCharacters]);
